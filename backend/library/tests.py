...
class LoanBoundaryTestCase(TestCase):
    ...
    def _create_book(self, available_count):    # テスト用ヘルパーメソッド（_で始める慣例）
        self._book_counter = getattr(self, '_book_counter', 0) + 1
        # getattr で _book_counter が未定義のとき 0 を返し、1 加算して連番を作る
        # setUp ごとにインスタンスが作り直されるのでカウンターはテストメソッドごとにリセットされる
        return Book.objects.create(
            title='テスト本',
            author='テスト著者',
            publisher='テスト出版社',
            isbn=f'978400000{self._book_counter:04d}',  # 連番で一意なISBNを生成する
            category=self.category,
